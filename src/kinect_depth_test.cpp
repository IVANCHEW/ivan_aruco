...
class DataManagement
{
	private:
		...
		std::vector <cv::Point2f> pixel_point;
	...
	public:
		...
		bool getPointCloudIndex(int &index);
...
void DataManagement::loadPixelPoint(cv::Point2f p){
	//~ std::cout << "Pixel point received: " << p << std::endl;
	//~ int i = p.y*frame_width + p.x;
	//~ std::cout << "Index: " << i << sstd::endl;
	//~ if (!pixel_point_ready){
		pixel_point = p;
		if (parameters_ready){
			cloud_index = pixel_point.y * frame_width + pixel_point.x;
			//~ std::cout << "Cloud index: " << cloud_index << std::endl;
			if (cloud_ready && transformation_ready){
				std::cout << "cloud 3D data. X: " << cloud->points[cloud_index].x << " Y: " << cloud->points[cloud_index].y << " Z: " << cloud->points[cloud_index].z << std::endl;
				std::cout << "Estimated Transformation: " << tvec << std::endl;
			}
		}
...
bool DataManagement::getPointCloudIndex(int &index){
...
void DataManagement::clearPixelPoints(){
	std::vector <int> ().swap(pixel_index);
	std::vector <cv::Point2f> ().swap(pixel_point);
}
...
void *start_cloud_viewer(void *threadid){
  ...
  while (!viewer.wasStopped ()){
    ...
    retrieve_index_ = dm.getPointCloudIndex(cloud_index);
    ...
    if (retrieve_cloud_){
			if (!update_){
				...
				if (retrieve_index_){
					//~ std::cout << "Index retrieved, updating highlights" << std::endl;
					//Display distinguished key point
					highlight_cloud->points[0] = cloud_a->points[cloud_index];
