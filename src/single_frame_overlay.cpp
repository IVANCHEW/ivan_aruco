...
cv::Mat dist_coeffs;
std::string calibration_file = "calibration";
...
int image_height = 2322;
int image_width = 4128;
...
void init_texture(void){
	...
	std::cout << std::endl << "Initiating Texture" << std::endl;
	
	std::cout << std::endl << "Creating static mat texture" << std::endl;
	mat_vertex = cv::imread("single_frame/1.jpg");  
...
void update_texture(void){
	...
	if (frame_ready){
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, image_width, image_height,  0, GL_BGR, GL_UNSIGNED_BYTE, mat_vertex.ptr());
	}
}
...
void loadImage(){
	...
	try{
		...
		cv::Mat tvec;
		bool marker_found;
		image = cv::imread("single_frame/1.jpg");  
  	...
  	marker_found = arucoPoseEstimation(image, target_id, tvec, rvec, camera_matrix, dist_coeffs, true);
		...
		//~ cv::imshow("Kinect Image Viewer", image);
		//~ cv::waitKey(30);
	}
...
int main (int argc, char** argv){
  ...
  std::cout << "Single Frame Overlay" << std::endl;
  ros::init(argc, argv, "single_frame_overlay_viewer");
